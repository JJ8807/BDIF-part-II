...
path = 'C:/Users/dell/Desktop/Graduate School Study/Spring 2020/Big Data in Finance/Part2/liramota-big_data2020/homeworks/hm_ii/liramota-fire_pytools'
...
def calculate_cumulative_returns(mdata, tt, min_periods): # TODO: to be completed
    ...
    cr = df.groupby('permno').retadj.rolling(window = tt, min_periods = min_periods).apply(np.prod, raw = True)
    cr = cr.groupby('permno').shift(1)
    cr = cr.to_frame('ret_' + str(11)+'_1').reset_index(level = 0)
    df['ret_' + str(tt)+'_1'] = cr['ret_' + str(tt)+'_1'] - 1
    #a['ret_' + str(11)+'_1']
    cret = pd.DataFrame(columns = ['cr'])
    cret.cr = df['ret_' + str(tt)+'_1'] # You should calculate the cumulative returns.
...
def main(save_out=True):
    ...
    DATAPATH = "C:/Users/dell/Desktop/Graduate School Study/Spring 2020/Big Data in Finance/Part2/liramota-big_data2020/homeworks/hm_ii/output/" # where to save output?
    ...
    crspm['ret_' + str(11)+'_1'] = calculate_cumulative_returns(crspm,11,1)['cr']# TODO: Calculate past 11, 1 returns
    crspm['rankyear'] = crspm.date.dt.year
    crspm.loc[crspm.date.dt.month <= 6, 'rankyear'] = crspm.loc[crspm.date.dt.month <= 6, 'rankyear'] - 1

    if save_out:
        crspm.to_pickle(DATAPATH+'stock_monthly1.pkl')
